...
\begin{definition}
An earthquake hits a board $b$ at node $s$, and the resulting board is denoted $E_sb$. If $b(s) \geq 4$, then
\begin{align*}
E_sb(s) &= b(s)-4,\\
...
\begin{definition} 
We say that a board is finite if the total number of chips on it is finite. Otherwise we say a board is infinite. 
\end{definition}
...
\caption{Gates in the simple model. Green are the inputs, red are the outputs}
