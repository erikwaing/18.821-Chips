...
The motivation for this definition is that the spread function should give a measure of how ``spread out" the chips on a board are. Intuitvely, earthquakes have the function of moving chips that are condensed in a single piile and spreading them out over a greater area. This function, which sums up the chip stack sizes weighted by the taxicab distance squared to the origin, sufficiently captures the idea of spread.



\begin{theorem}
\label{finitestability}
If a board $b$ is finite then there exists $N>0$ such that after applying a $N$ earthquakes the resultant board will be stable.
\end{theorem}
...
\begin{lemma}
Suppose $b$ is a finite board. If $E_{s}b \neq b$, then no application of earthquakes to $E_{s}b$ can produce $b$. 
\end{lemma}

\begin{proof}
For ease of notation we use $s=(s_1, s_2) \in \mathbb{Z} \times \mathbb{Z}$. We define our spread function as a map from finite boards to the nonnegative integers
\begin{equation*}
S(b) = \sum_{i,j} b(i,j)(|i|+|j|)^2. 
\end{equation*}
We can think of $S(b)$ as the sum of the number of chips on each square weighted by the "taxi-cab" distance from the origin squared. This is well-defined since the total number of chips on the board is finite. We claim that 
\begin{equation*}
...
We evaluate $S(E_{s} b) - S(b)$, recalling that $E_s b(i,j)$ refers to the value of $b$ at $(i,j)$ after an earthquake effected $s$.
\begin{align*}
S(E_{s} b) - S(b) &=\sum_{i,j} E_{s} b (i,j)(|i|+|j|)^2 - \sum_{i,j} b(i,j)(|i|+|j|)^2 \\
&=-4(|s_1| + |s_2|)^2 +\sum_{(i,j) \in n(s)} (|i| + |j|)^2
\end{align*}
For at most two of the neighbors of $(s_1, s_2)$ 
\begin{equation*}
(|i|+|j|)= (|s_1|+|s_2|-1)^2,
\end{equation*}
