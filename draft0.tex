...
=======
We also consider extensions of this problem set up where the earthquake hits all squares on the board at once. We refer to the former type of earthquake as $\textbf{small earthquakes}$ and earthquakes that hit the entire board as $\textbf{big earthquakes}$.
\section{Standard Board}

Written by Erik Waingarten and Perry Kleinhenz, edited by Fermi Ma
>>>>>>> 3c0ff89f43d12d793e71858c071194622ad23251
...
We use the notation $SE^n(f, \{(x_i,y_i)\})$ to refer to the application of $SE$ to the board $f$, $n$ times to the squares $(x_1, y_1), \ldots (x_{n}, y_{n})$ respectively. 
\end{definition}
...
If the initial board has a finite number of chips, there exists an $N$ such that after $N$ big earthquakes or $N$ small earthquakes the resultant board will be stable.
\end{theorem}
...
A restatement of the theorem is to say that for all $B \in \mathcal{B}_f$ there exists an $N$ such that for all $m \in \mathbb{Z}_{\geq 0}$, $BE^N(f) = BE^{N+m}(f)$ and $SE^{M}(f) = SE^{M+k}(f)$
...
That is if big earthquake changes the state of the board, then $\Phi(f)$ increases, therefore if $E(f) \neq f$, then for all $N > 0$, $E^N(f) \neq f$, or in other words that you never return to your original state.
...
Suppose $f(x_0,y_0) \geq 4$ for some location $(x_0, y_0)$, then the number of chips in location $(x_0,y_0)$ will have decreased by $4$ after applying $BE$. 
...
So
\[ \Phi(BE(f)) - \Phi(f) = \sum_{(x,y) \in A} \left((-4)(|x|+|y|)^2 + \sum_{(v,w) \in n(x,y)} (|v| + |w|)^2 \right) \]
...
\[ \Phi(f) = \sum_{x,y} f(x,y)(|x|+|y|)^2, \]
and evaluate $\Phi(SE(f, (x_1,y_1))) - \Phi(f)$:
\begin{align}
\Phi(SE(f), (x_1, y_1)) - \Phi(f) =\sum_{x,y} SE(f(x,y), (x_1, y_1))(|x|+|y|)^2 - \sum_{x,y} f(x,y)(|x|+|y|)^2 \\
= SE(f(x_1,y_1), (x_1, y_1))(|x_1|+|y_1|)^2 + \sum_{(x,y) \in n(x_1,y_1)} SE(f(x,y), (x_1, y_1))(|x|+|y|)^2 \\- f(x_1,y_1)(|x_1|+|y_1|)^2 + \sum_{(x,y) \in n(x_1, y_1)} f(x,y)(|x|+|y|)^2 \\
...
\begin{proof}
(of Theorem~\ref{finitestability}) Suppose $f \in \mathcal{B}_f$, so $\sum_{x,y} f(x,y) = S$, and suppose all chips lie within the locations $[x_l, x_h] \times [y_l, y_h]$. Then by Lemma~\ref{finiteextension}, we have that for all $n$ all chips on $SE^n(f)$ and $BE^n(f)$ will lie in $R = [x_l - \sqrt{S}, x_h + \sqrt{S}] \times [y_l - \sqrt{S}, y_h + \sqrt{S}]$. 

There exists less than $(x_h - x_l + 2\sqrt{S})(y_h - y_l + 2\sqrt{S})S$ possible boards $b \in \mathcal{B}_f$ that satisfy this. We know that there are no cycles in the states, so there must be an $0 \leq N < (x_h - x_l + 2\sqrt{S})(y_h - y_l + 2\sqrt{S})S$ such that $SE^N(f) = SE^{N+m}(f)$ and $BE^N(f)=BE^{N+m}$ for all $m \geq 0$.
\end{proof}
...
\begin{definition}
We define a valid earthquake to be a local earthquake that occurs on a square that, at the time of the earthquake, has at least 4 chips.

We say that a sequence of local earthquakes $LE_1, LE_2, \ldots$ to be a valid sequence of earthquakes if each earthquake is valid.
\end{definition}
