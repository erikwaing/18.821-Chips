...
We then use the fact that local earthquakes strictly increase the value of this spread function to show that finite boards cannot cycle back to previous configurations, from which we will conclude that all finite boards reach a steady state. 
In Section~\ref{Acyclicity of Infinite Boards}, we modify the spread function so that it converges to a real number even for infinite boards, with bounded chip stack size. 
...
\begin{definition}
An earthquake takes as input a board and the coordinates of a grid square and outputs a board, so we write $E: \mathcal{B} \times (\mathbb{Z} \times \mathbb{Z}) \rightarrow \mathcal{B}$. If $b(x) \geq 4$, then
\begin{align*}
E[ b, x](x) = b(x)-4 \\
E[ b, x](y) = b(y)+1 \\
\end{align*}
for $ y \in n(x)$. Otherwise  $b(x,y) < 4$, and $E[b] = b$, as the earthquake does not move any chips.
...
A global earthquake (abbreviated as $GE$) is similar to a standard earthquake applied to every square on the board simultaneously.
...
\begin{align*}
GE(f) = \sum_{i, j \in \mathbb{Z}\times \mathbb{Z}} E(f, (i,j))
\end{align*}
\end{definition}
...
\end{definition}
This is equivalent to saying an earthquake has no effect on the board. We would also like to formalize the definition of a finite board. 

\begin{definition} 
We say that a board is finite if the total number of chips on it is finite. That is a board $f$ is finite if the sum
\begin{equation}
C= \sum_{(x,y) \in \mathbb{Z} \times \mathbb{Z}} f(x,y) 
\end{equation}
...
\begin{proof}
For ease of notation we use $x=(x_1, x_2)$. We define our spread function as $S: \mathcal{B}_f \rightarrow \mathbb{Z}_{\geq 0}$ 
\begin{equation*}
S(f) = \sum_{i,j} b(i,j)(|i|+|j|)^2. 
\end{equation*}
...
\begin{equation*}
E[f, x] \neq f \Rightarrow S(E[f, x]) > S(f)
\end{equation*}
That is if an earthquake changes the board, $S$ increases. This implies if $E[b, x] \neq b$, then $E^n[b, \{x_n\}] \neq b$ for all $n > 0$ and all sequences $\{x_n\}$ . In other words, a series of earthquakes applied to a finite board can never produce a previous board. 

We evaluate $S(E[b, x]) - S(f)$, recalling that $E[b, x](i,j)$ refers to the value of $b$ at $(i,j)$ after an earthquake effected $x$.
\begin{align*}
&S(E[b, x]) - S(f) =\sum_{i,j} E[b, x](i,j)(|i|+|j|)^2 - \sum_{i,j} b(i,j)(|i|+|j|)^2 \\
&= E[b, x](x_1,x_2)(|x_1|+|x_2|)^2 + \sum_{(i,j) \in n(x_1,x_2)} E[b, x] (i,j) (|i|+|j|)^2 \\
&- b(x_1, x_2)(|x_1|+|x_2|)^2 + \qquad \;  \; \sum_{(i,j) \in n(x_1, x_2)} b(i,j)(|i|+|j|)^2 \\
&= 4(|x_1| + |x_2|)^2 +\qquad \qquad \quad \; \; \sum_{(i,j) \in n(x_1, x_2)} (|i| + |j|)^2,
\end{align*}
At most two of the neighbors of $(x_1, x_2))$ are a taxicab distance $(|x_1|+|x_2|-1)^2$ from the origin and at least two of the neighbors are a taxicab distance $(|x_1|+|x_2|+1)^2$ from the origin and $(|x_1|+|x_2|+1)^2 > (|x_1|+|x_2|-1)^2$. Therefore 
\begin{align*}
&4(|x_1| + |x_2|)^2 +\sum_{(i,j) \in n(x_1, x_2)} (|i| + |j|)^2 \\
& \geq 4(|x_1| + |x_2|)^2 + 2 (|x_1| + |x_2|+1)^2 + 2 (|x_1| + |x_2|-1)^2 = 4 >0.
\end{align*}
\end{proof}
...
Suppose $\sum_{x,y} b(x,y) = C$, and $f$ is nonzero only in the range $[i_{min}, i_{max}] \times [j_{min}, j_{max}]$. Then   $E^n[b, \{x_n\}$ is nonzero only in the range $[i_{min}-C, i_{max}+C] \times [j_{min}-C, j_{max}+C]$for all $n$ and for all sequences $\{x_n\}$ .
\end{lemma}
...
\begin{proof}
(of Theorem~\ref{finitestability}) Recall that $b \in \mathcal{B}_f$, so $\sum_{x,y} f(x,y) = C$. Because $b$ is a finite board we know the locations with a nonzero number of chips are bounded.  Without loss of generality we can say that $b$ is nonzero only in the range $[i_{min}, i_{max}] \times [j_{min}, j_{max}]$. Then by Lemma~\ref{finiteextension}, we have that $E^n[f, \{x_n\}]$ is nonzero only in the range $ R=[i_{min}-C, i_{max}+C] \times [j_{min}-C, j_{max}+C]$, for all $n>0$ and all sequences $\{ x_n \}$.
If we set 
\begin{equation*}
D= |j_{max}-j_{min} + 2C|*|i_{max} - i_{min} + 2C|
\end{equation*}
then there are at most  $\binom{D+C-1}{D-1}$  boards that have exactly $C$ chips on them and are nonzero only in $R$,. We know that the state of the board cannot cycle, so there must be an $0 \leq N \leq \binom{D+C-1}{D-1}$ and some sequence $\{x_n\}$ such that $E^N[ f, \{x_n\}] = LE^{N+m}[f, \{x_n\}]$. 
\end{proof}
