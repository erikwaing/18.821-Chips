...
\label{fig:growthofT}
\end{figure}
By looking at Figure~\ref{fig:growthofT} seems that $T(n) = \Theta(n\log n)$. Our strategy to show a lower bound on $T(n)$ by giving a lower bound on $T'(n)$ seems to be the correct approach, since by experimental evidence, we believe that $T'(n) = \Theta(n\log n)$.
