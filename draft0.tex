...
If the initial board has a finite number of chips, there exists an $N$ and $M$ such that after $N$ global earthquakes or $M$ local earthquakes the resultant board will be stable.
\end{theorem}
...
So a restatement of the theorem is to say that for all $B \in \mathcal{B}_f$ there exists an $N$ and $M$ such that for all $m \in \mathbb{Z}_{\geq 0}$, $GE^N(f) = GE^{N+m}(f)$ and for all $k \in \mathbb{Z}_{\geq 0}$, $LE^{M}(f) = LE^{M+k}(f)$
...
So
\[ \Phi(GE(f)) - \Phi(f) \geq \sum_{(x,y) \in A} (-4)(|x|+|y|)^2 + 2(|x|+|y|+1)^2 + 2(|x|+|y|-1)^2 \]
...
\[ \Phi(f) = \sum_{x,y} f(x,y)(|x|+|y|)^2, \]
and evaluate $\Phi(LE(f)) - \Phi(f)$. Suppose $LE$ effected square $(x_0,y_0)$ and its neighbors, then
\begin{align}
\Phi(LE(f)) - \Phi(f) =\sum_{x,y} LE(f(x,y))(|x|+|y|)^2 - \sum_{x,y} f(x,y)(|x|+|y|)^2 \\
= LE(f(x_0,y_0))(|x_0|+|y_0|)^2 + \sum_{(x,y) \in n(x_0,y_0)} LE(f(x,y))(|x|+|y|)^2 \\- f(x_0,y_0)(|x_0|+|y_0|)^2 + \sum_{(x,y) \in n(x_0,y_0)} f(x,y)(|x|+|y|)^2 \\
